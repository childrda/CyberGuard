...
class ReportWebhookController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        try {
            ...
            $userActions = $this->normalizeWebhookUserActions($data['user_actions'] ?? null);
    ...
    /**
     * Gmail add-on may send checkbox values as strings, nested arrays, or objects.
     *
     * @return list<string>
     */
    private function normalizeWebhookUserActions(mixed $raw): array
    {
        $flat = $this->flattenWebhookUserActionValues($raw);
        $out = [];
        foreach ($flat as $s) {
            $s = trim((string) $s);
            if ($s !== '') {
     ...
     * @return list<string>
     */
    private function flattenWebhookUserActionValues(mixed $raw): array
    {
        if ($raw === null || $raw === '') {
            return [];
        }
        if (is_string($raw)) {
            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                return $this->flattenWebhookUserActionValues($decoded);
            }
        ...
        foreach ($raw as $key => $value) {
            ...
            if (is_array($value)) {
                $picked = false;
                foreach (['value', 'Value', 'selectedValue', 'name', 'id'] as $attr) {
                    if (isset($value[$attr]) && is_string($value[$attr]) && trim($value[$attr]) !== '') {
                        $out[] = trim($value[$attr]);
                        $picked = true;
                        break;
                    }
                ...
                if (! $picked) {
                    $out = array_merge($out, $this->flattenWebhookUserActionValues($value));
                }

                continue;
            }
            if ($value === true && is_string($key) && trim($key) !== '') {
                $out[] = trim($key);
            }
        }
