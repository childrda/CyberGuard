...
<div class="mb-6">
    ...
    <span class="mt-2 inline-block">@include('admin.remediation.partials.status-badge', ['job' => $job])</span>
</div>

@if($job->completed_at)
<div class="rounded-lg border border-slate-200 bg-white p-6 mb-6">
    <h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Summary</h2>
    <dl class="grid gap-2 text-sm sm:grid-cols-2 md:grid-cols-4">
        <div><dt class="text-slate-500">Removed</dt><dd class="font-medium">{{ $job->removed_count ?? 0 }}</dd></div>
        <div><dt class="text-slate-500">Skipped</dt><dd class="font-medium">{{ $job->skipped_count ?? 0 }}</dd></div>
        <div><dt class="text-slate-500">Dry run (simulated)</dt><dd class="font-medium">{{ $job->dry_run_count ?? 0 }}</dd></div>
        <div><dt class="text-slate-500">Failed</dt><dd class="font-medium">{{ $job->failed_count ?? 0 }}</dd></div>
    </dl>
    @if($job->failure_summary)
        <p class="mt-3 text-sm text-amber-700">{{ $job->failure_summary }}</p>
    @endif
</div>
@endif
...
<div class="rounded-lg border border-slate-200 bg-white p-6 mb-6">
    ...
    <dl class="grid gap-2 text-sm">
        <div><dt class="text-slate-500">Dry run</dt><dd>{{ $job->dry_run ? 'Yes (simulated only)' : 'No' }}</dd></div>
